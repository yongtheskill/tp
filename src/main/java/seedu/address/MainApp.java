...
public class MainApp extends Application {
    ...
    public void init() throws Exception {
        logger.info("=============================[ Initializing PingBook ]===========================");
        super.init();
        ...
        JsonPingBookStorage pingBookStorage = new JsonPingBookStorage(userPrefs.getAddressBookFilePath());
        storage = new StorageManager(pingBookStorage, userPrefsStorage, pingBookStorage);
    ...
    /**
     * Returns a {@code ModelManager} with the data from {@code storage}'s PingBook file and {@code userPrefs}. <br>
     * The data from the sample PingBook will be used instead if {@code storage}'s PingBook file is not found,
     * or an empty PingBook will be used instead if errors occur when reading {@code storage}'s PingBook file.
     */
        ...
        try {
            ...
            if (!addressBookOptional.isPresent()) {
                logger.info("Creating a new data file " + storage.getAddressBookFilePath()
                    + " populated with a sample PingBook.");
            }
            initialData = addressBookOptional.orElseGet(SampleDataUtil::getSampleAddressBook);
        } catch (DataLoadingException e) {
            logger.warning("Data file at " + storage.getAddressBookFilePath() + " could not be loaded."
                    + " Will be starting with an empty PingBook.");
    ...
    public void start(Stage primaryStage) {
        logger.info("Starting PingBook " + MainApp.VERSION);
        ui.start(primaryStage);
    }

    @Override
    public void stop() {
        logger.info("============================ [ Stopping PingBook ] =============================");
        try {
